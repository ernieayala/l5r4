...
export default class L5R4PcSheet extends BaseActorSheet {
  ...
  async _onXpModal(event) {
        ...
        .map(e => {
          ...
          if (e.type === "trait" && e.traitLabel && e.toValue !== undefined) {
            ...
            if (formattedNote.includes("l5r4.character.experience.log.traitChange")) {
              type = game.i18n.localize("l5r4.character.experience.breakdown.traits");
              // The note is just the localization key, we need to look at the original parameters
              // For now, show a generic trait message until we can parse the actual data
              ...
              // Default type for manual entries
    ...
    const content = await renderTemplate("systems/l5r4/templates/chat/xp-modal.hbs", templateData);
    ...
    const dialog = new Dialog({
      ...
      render: (html) => {
        ...
        this._setupXpModalListeners(html);
      }
    }, {
      width: 600,
      height: 700,
  ...
  /**
   * Setup event listeners for the XP modal
   * @param {jQuery} html - The modal HTML
   */
  _setupXpModalListeners(html) {
    // XP Add button
    html.find('[data-action="xp-add-confirm"]').click(async (event) => {
      event.preventDefault();
      const amount = Number(html.find('#xp-amount').val()) || 0;
      const note = html.find('#xp-note').val()?.trim() || "";
      ...
      try {
        ...
        this._refreshXpModal(html);
      } catch (err) {
        console.warn("L5R4", "actor.setFlag failed in PcSheet", { err });
      }
    });

    // Delete manual entry buttons
    html.find('[data-action="xp-delete-manual"]').click(async (event) => {
      event.preventDefault();
  ...
  /**
   * Refresh the XP modal content without creating a new dialog
   * @param {jQuery} html - The modal HTML
   */
  async _refreshXpModal(html) {
    // Get updated XP data
    const sys = this.actor.system ?? {};
    const xp = sys?._xp ?? {};
    const ns = this.actor.flags?.[SYS_ID] ?? {};
    ...
    // Format entries for display (copied from original _onXpModal)
    const formatEntries = (arr) =>
      arr
        .slice()
        .sort((a, b) => (a.ts || 0) - (b.ts || 0))
        .map(e => {
          ...
          if (e.type === "trait" && e.traitLabel && e.toValue !== undefined) {
            ...
            if (formattedNote.includes("l5r4.character.experience.log.traitChange")) {
              type = game.i18n.localize("l5r4.character.experience.breakdown.traits");
              formattedNote = "Trait Increased";
            } else if (formattedNote.includes("l5r4.character.experience.log.voidChange")) {
              type = game.i18n.localize("l5r4.character.experience.breakdown.void");
              formattedNote = "Void Increased";
            } else if (formattedNote.includes("l5r4.character.experience.log.skillChange")) {
              type = game.i18n.localize("l5r4.character.experience.breakdown.skills");
              formattedNote = "Skill Increased";
            } else if (e.type) {
    ...
    // Re-render the template content
    const newContent = await renderTemplate("systems/l5r4/templates/chat/xp-modal.hbs", templateData);
    ...
    html.find('#xp-amount').val(1);
    html.find('#xp-note').val('');
  }
