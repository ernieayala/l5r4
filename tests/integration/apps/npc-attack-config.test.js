/**
 * @fileoverview NPC Attack Config Integration Tests
 *
 * Tests the NPC Attack Config application for editing attack and damage values.
 *
 * **Test Coverage:**
...
import NpcAttackConfigApplication from "../../../module/apps/npc-attack-config.js";

/**
 * Register NPC Attack Config tests
 * @param {Object} quench - Quench test framework API
 */
export function registerNPCAttackConfigTests(quench) {
  quench.registerBatch(
    `${SYS_ID}.apps.npc-attack-config`,
    context => {
      const { describe, it, assert, beforeEach, afterEach } = context;

      describe("NPC Attack Config Rendering", () => {
        let actor, editor;
        ...
        beforeEach(async () => {
          ...
          editor = new NpcAttackConfigApplication({
            actor,
            attackKey: "attack1"
          });
        });
      ...
      describe("NPC Attack Config Form Behavior", () => {
        let actor, editor;
      ...
      describe("NPC Attack Config Data Updates", () => {
        let actor, editor;
      ...
      describe("NPC Attack Config for Different Attack Slots", () => {
        let actor;
        ...
        it("should open editor for attack1", async () => {
          const editor = new NpcAttackConfigApplication({
            actor,
            attackKey: "attack1"
          });
    ...
    { displayName: "L5R4: NPC Attack Config Tests" }
  );
}
